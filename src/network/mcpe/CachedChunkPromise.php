...
class CachedChunkPromise{
	...
	private ?CachedChunk $result = null;
	...
	public function onResolve(\Closure ...$callbacks) : void{
		...
		foreach($callbacks as $callback){
			Utils::validateCallableSignature(function(CachedChunkPromise $promise) : void{}, $callback);
		}
		if($this->result !== null){
			foreach($callbacks as $callback){
				$callback($this);
	...
	public function resolve(CachedChunk $result) : void{
		if(!$this->cancelled){
			if($this->result !== null){
	...
	public function getResult() : CachedChunk{
		$this->checkCancelled();
		if($this->result === null){
